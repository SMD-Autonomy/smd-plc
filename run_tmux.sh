...
then
    ...
    # Run the plc subscriber in Pane 1 (left top)
    tmux send-keys -t $SESSION:0.0 'cd app; ./objs/x64Linux4gcc7.3.0/plc_subscriber -d 6' C-m  
        
    # Run the BEAM plc publisher in Pane 2 (right top)
    tmux send-keys -t $SESSION:0.1 'cd publisher_control; python3 plc_publisher.py --domain_id 6.0 --sample_count 1 --camera_id 9.0 --camera_power True --camera_light True --camera_focus 22.0 --camera_zoom 25.0 --lamp_id 9.0 --lamp_intensity 80.0 --lamp_power True --panandtilt_id 9.0 --panandtilt_x 80.0 --panandtilt_y 20.0 --panandtilt_z 23.0' C-m  

    # Run the gateway (OPCUA) in Pane 2 (right bottom)
