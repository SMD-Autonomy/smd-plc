...
then
    ...
    # Run the gateway in Pane 0 (left top)    
    tmux send-keys -t $SESSION:0.0 'python3 plc.py' C-m    

    # Run the camera server (OPCUA) in Pane 1 (right top)
    tmux send-keys -t $SESSION:0.1 'cd app; ./objs/x64Linux4gcc7.3.0/plc_subscriber -d 11' C-m  
